...
class SettingsViewController: UIViewController {
    ...
    func setupData() {
        ...
        let control = CellModel(title: "空降助手广告屏蔽", desp: Settings.enableSponsorBlock.title) { [weak self] in
            let alert = UIAlertController(title: "空降助手广告屏蔽", message: nil, preferredStyle: .actionSheet)
            for blockType in SponsorBlockType.allCases {
                let action = UIAlertAction(title: blockType.title, style: .default) { _ in
                    Settings.enableSponsorBlock = blockType
                    self?.setupData()
                }
                alert.addAction(action)
            }
        ...
        let danmuFilter = CellModel(title: "用户自定义弹幕屏蔽", desp: Settings.enableDanmuFilter ? "开" : "关") {
            [weak self] in
            Settings.enableDanmuFilter.toggle()
